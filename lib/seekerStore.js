...
export async function saveResume({ fileName, size, type, dataUrl }) {
  ...
  if (getSeekerToken()) {
    try {
      await seekerApi.saveSeekerResume({ fileName, size, dataUrl });
    } catch (e) {
...
export function isSaved(jobId) {
  ...
  return list.includes(jobId) || list.includes(String(jobId));
}
...
export async function toggleSavedJob(jobId) {
  ...
  const has = store.savedJobs.some((id) => String(id) === idStr);
  store.savedJobs = has
    ? store.savedJobs.filter((id) => String(id) !== idStr)
