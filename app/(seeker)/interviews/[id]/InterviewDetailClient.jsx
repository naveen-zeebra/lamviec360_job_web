    ...
    <div className={PAGE}>
      ...
      <div className="mb-5 flex flex-wrap items-start gap-3">
        <div className="flex h-14 w-14 flex-shrink-0 items-center justify-center rounded-md bg-blue-600 text-sm font-bold text-white">
          {job ? (job.company_name || job.company || "—").slice(0, 2).toUpperCase() : "—"}
        </div>
        <div className="min-w-0 flex-1">
          <h1 className="text-xl font-extrabold">{title}</h1>
          <p className="text-sm text-muted">{job ? job.company_name || job.company : ""} · {iv.round}</p>
        </div>
      ...
      <div className={`${CARD} mb-5`}>
        ...
        <div className="mt-4 flex flex-wrap gap-2.5">
          ...
          <Button variant="secondary" size="sm" onClick={() => downloadIcs(iv, title, job ? job.company_name || job.company : "")}>
